...
document.addEventListener("DOMContentLoaded", function () {
    const dropdownButtons = document.querySelectorAll('.heads');
    const dropdownContents = document.querySelectorAll('.dropdown-content');
    
    dropdownButtons.forEach(button => {
        button.addEventListener('click', function () {
            const contentClass = `.dropdown-content-${this.id.replace('Btn', '')}`;
            const content = document.querySelector(contentClass);

            // Hide all dropdowns and remove body overflow
            dropdownContents.forEach(c => c.classList.remove('show'));
            document.body.style.overflow = '';

            // Show the clicked dropdown and add body overflow hidden
            if (content) {
